...
var (
	schema = []string{
		...
		`CREATE INDEX IF NOT EXISTS ` + generic.TableName + `_name_index ON ` + generic.TableName + ` (name)`,
		`CREATE INDEX IF NOT EXISTS ` + generic.TableName + `_name_id_index ON ` + generic.TableName + ` (name,id)`,
		`CREATE INDEX IF NOT EXISTS ` + generic.TableName + `_id_deleted_index ON ` + generic.TableName + ` (id,deleted)`,
		`CREATE INDEX IF NOT EXISTS ` + generic.TableName + `_prev_revision_index ON ` + generic.TableName + ` (prev_revision)`,
		`CREATE UNIQUE INDEX IF NOT EXISTS ` + generic.TableName + `_name_prev_revision_uindex ON ` + generic.TableName + ` (name, prev_revision)`,
		`CREATE INDEX IF NOT EXISTS ` + generic.TableName + `_list_query_index on ` + generic.TableName + `(name, id DESC, deleted)`,
	}
	schemaMigrations = []string{
		`ALTER TABLE ` + generic.TableName + ` ALTER COLUMN id SET DATA TYPE BIGINT, ALTER COLUMN create_revision SET DATA TYPE BIGINT, ALTER COLUMN prev_revision SET DATA TYPE BIGINT; ALTER SEQUENCE ` + generic.TableName + `_id_seq AS BIGINT`,
...
func New(ctx context.Context, wg *sync.WaitGroup, cfg *drivers.Config) (bool, server.Backend, error) {
	...
	dialect.InsertRetry = func(err error) bool {
		if err, ok := err.(*pgconn.PgError); ok && err.Code == pgerrcode.UniqueViolation && err.ConstraintName == generic.TableName+"_pkey" {
			return true
		}
		return false
	}
